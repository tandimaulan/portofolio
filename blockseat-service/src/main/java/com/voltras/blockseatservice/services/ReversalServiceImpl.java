...
public class ReversalServiceImpl implements RpcBasicService, ReversalService{
	@Autowired
	...
	public List<ReversalDetail> pull() {
		...
		repo.saveAll(datas);
		return datas.stream()
				.map(data -> new ReversalDetail(data.getGoblinAccountId(), data.getTransactionId(), "BLOCKSEAT")).toList();
	}
}
